  ...
  return (
    <div>
      <div className="flex justify-between mb-1">
        ...
        <span className="text-[10px] font-mono text-[#2a2a2a]">{Math.round(value)}</span>
      </div>
      <input
        type="range" min={0} max={max} step={1} value={value}
        onChange={(e) => onChange(Number(e.target.value))}
        className="w-full h-2 appearance-none cursor-pointer rounded-sm border border-[#d0cdc4] [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-3 [&::-webkit-slider-thumb]:h-3 [&::-webkit-slider-thumb]:bg-white [&::-webkit-slider-thumb]:border [&::-webkit-slider-thumb]:border-[#2a2a2a] [&::-webkit-slider-thumb]:cursor-pointer [&::-moz-range-thumb]:w-3 [&::-moz-range-thumb]:h-3 [&::-moz-range-thumb]:bg-white [&::-moz-range-thumb]:border [&::-moz-range-thumb]:border-[#2a2a2a] [&::-moz-range-thumb]:cursor-pointer"
  ...
  return (
    <div className="flex items-center justify-between">
      ...
      <button
        ref={triggerRef}
        onClick={toggle}
        className="w-8 h-8 border border-[#d0cdc4] hover:border-[#2a2a2a]"
      ...
      {open && mounted && createPortal(
        <>
          ...
          <div
            className="fixed z-[61] w-[248px] p-3 bg-[#f5f3ee] border border-[#2a2a2a] shadow-xl space-y-3"
            style={{ top: pos.top, left: pos.left }}
          >
            <div className="text-[10px] text-[#999] uppercase tracking-wide">{label}</div>

            <div className="flex items-center gap-2">
              <div className="w-9 h-9 border border-[#d0cdc4] shrink-0" style={{ backgroundColor: hex }} />
              <input
                value={hexText}
                ...
                spellCheck={false}
                className="flex-1 min-w-0 p-1.5 bg-[#e8e5dd] border border-[#d0cdc4] text-xs font-mono text-[#2a2a2a] uppercase focus:border-[#2a2a2a] outline-none"
                maxLength={7}
              />
              <div className="relative w-9 h-9 shrink-0" title="System picker">
                ...
                <div className="w-9 h-9 border border-[#d0cdc4] flex items-center justify-center text-[#666] text-sm pointer-events-none">◐</div>
              </div>
            </div>
            ...
            <div>
              <div className="text-[10px] text-[#999] mb-1">Tint / shade</div>
              <div className="flex gap-1">
                {lightnessRamp(hex, 6).map((c, i) => (
                  <button key={i} onClick={() => apply(c)} title={c}
                    className="flex-1 h-5 border border-[#d0cdc4] hover:border-[#2a2a2a]" style={{ backgroundColor: c }} />
                ))}
              </div>
            </div>

            <div>
              <div className="text-[10px] text-[#999] mb-1">Harmony</div>
              <div className="flex gap-1">
                {harmonies(hex).map((h, i) => (
                  <button key={i} onClick={() => apply(h.hex)} title={`${h.label} · ${h.hex}`}
                    className="flex-1 h-5 border border-[#d0cdc4] hover:border-[#2a2a2a]" style={{ backgroundColor: h.hex }} />
                ))}
              </div>
            </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                <span className="text-[10px] text-[#999]">Saved</span>
                <button onClick={() => addSavedColor(hex)} className="text-[10px] text-[#2a2a2a] hover:underline">+ Save</button>
              </div>
              <div className="flex flex-wrap gap-1.5 min-h-[20px]">
                {savedColors.length === 0 && <span className="text-[10px] text-[#bbb]">No saved colours yet</span>}
                {savedColors.map((c) => (
                  <div key={c} className="relative group">
                    <button onClick={() => apply(c)} title={c}
                      className="w-5 h-5 border border-[#d0cdc4] hover:border-[#2a2a2a]" style={{ backgroundColor: c }} />
                    <button onClick={() => removeSavedColor(c)} aria-label="Remove"
                      className="absolute -top-1.5 -right-1.5 w-3.5 h-3.5 bg-[#2a2a2a] text-[#e8e5dd] text-[9px] leading-none flex items-center justify-center rounded-full opacity-0 group-hover:opacity-100">×</button>
                  </div>
                ))}
              </div>
            </div>
